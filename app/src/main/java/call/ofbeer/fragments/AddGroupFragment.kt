        ...
        btn_create.setOnClickListener{
            ...
            RetrofitClient.instance.newGroup(GroupRequest(name), session.TOKEN)
                .enqueue(object : retrofit2.Callback<GroupAddResponse>{
                    ...
                    override fun onResponse(call: Call<GroupAddResponse>,response: Response<GroupAddResponse>
                    ) {
                        if(response.code()==200)
                        {
                            Toast.makeText(requireContext(), "Grupa została utworzona!", Toast.LENGTH_SHORT).show()
