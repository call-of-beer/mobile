...
class GroupSeeDetailsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        usersInGroupList.layoutManager = LinearLayoutManager(requireContext())

        fetchUsers()

        startTasting.setOnClickListener{
        ...
        leaveGroup.setOnClickListener{
            ...
            RetrofitClient.instance.leaveGroup(session.TOKEN, session.goupID, session.userID)
                .enqueue(object : retrofit2.Callback<SuccesfulResponse>{
                    ...
                    override fun onResponse(call: Call<SuccesfulResponse>, response: Response<SuccesfulResponse>) {
                        if (response.code() == 200) {
                            Toast.makeText(context, "Użytkownik usunięty z grupy", Toast.LENGTH_SHORT).show()
                        }
                        if(response.code()==401){
                            Toast.makeText(context, "Jesteś moderatorem tej grupy, nie możesz jej opuścic.", Toast.LENGTH_SHORT).show()
                        }
                        else
            ...
            .enqueue(object : retrofit2.Callback<GroupInfoResponse>{
                ...
                override fun onResponse(call: Call<GroupInfoResponse>, response: Response<GroupInfoResponse>
                    ...
                    if (response.code() == 200) {
                        ...
                        usersInGroupList.adapter = userAdapter
